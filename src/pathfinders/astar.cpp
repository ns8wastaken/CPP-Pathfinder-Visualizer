...
int AStar::h(Cell* cell)
{
    // return std::abs(end->index.x - cell->index.x) + std::abs(end->index.y - cell->index.y); // Manhattan distance
    int a = end->index.x - cell->index.x;
    int b = end->index.y - cell->index.y;
    return (a * a) + (b * b);
}
...
char AStar::stepSearch()
{
    ...
    // Get cell with smallest f_cost
    for (std::vector<Cell*>::iterator it = open.begin(); it != open.end(); ++it) {
        Cell* cell = *it;
        if (cell->f_cost < current_cell->f_cost) {
            current_cell_it = it;
            current_cell = cell;
        }
    }
